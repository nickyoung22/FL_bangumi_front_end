          ...
          ">
            <video class="hover-active" controls controlsList="nodownload" disablePictureInPicture="true">
              <source v-bind:src="
                ...
                '/' +
                encodeURIComponent(
                  list_item_data.storeName + '/hots/' + hot_item
                )
              " />
              您的浏览器不支持 video 标签。
            </video>
        ...
        <div class="filenames-box-wrapper clearfix">
          ...
          <div class="filenames-box float-right">
            <template v-for="file in list_item_data.file_names">
              <File_icon @click="open(list_item_data.type, list_item_data.storeName, file)"
                class="file-names  click-active hover-active" :fileName="file" v-bind="{
                  url_exact: true,
