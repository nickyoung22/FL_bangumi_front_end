      ...
      <div v-show="show" class="field-data-box--buttons-box">
        <div class="field-data-box clearfix">
          <div class="tools float-left">
            <el-button type="primary" @click="uncollected_field">未收藏的{{ field_showName }}</el-button>
            <el-button type="primary" @click="empty_field_resource">无{{ field_showName }}的资源</el-button>
          </div>
          <template v-for="item in field_data">
            <div class="field-data-item float-left">
              <div class="top clearfix">
                <img class="float-left" v-bind:src="
                  store.api_server +
                  '/static/' +
                  type +
                ...
                <div class="icons-box  float-left">
                  <template v-for="file in item.file_names">
                    <File_icon class="hover-active click-active" @click="open(type, `##${field}`, item.storeName, file)"
                      :fileName="file"></File_icon>
                  </template>
                </div>
              </div>
              <div class="bottom clearfix">
                <el-tooltip effect="dark" :content="item.name.join('-')" placement="left-end" :hide-after="0">
                  <span @click="filter(item.name)" class="name float-left click-active hover-active">{{ item.name[0]
                  }}</span>
                </el-tooltip>
                ...
                <span class="folder float-right">
                  <File_icon class="hover-active click-active" @click="open(type, `##${field}`, item.storeName)" v-bind="{
                    fileName: '',
                    type_appoint: 'folder',
...
export default {
  ...
  methods: {
    ...
    uncollected_field() {
      ...
      let all = new Set(), collected = new Set(), uncollected = new Set()
...
.filter-box {
      ...
      .field-data-item {
        ...
        .top {
          ...
          .icons-box {
            ...
            border: 2px solid rgba(235, 255, 12, .366);
  ...
  .fold-triangle {
    width: 0;
    height: 0;
    border-left: calc(var(--right-sider-width) * .4) solid transparent;
    border-right: calc(var(--right-sider-width) * .4) solid transparent;
