      ...
      <ul class="ul-container">
        <li v-for="item in render_data" :key="type_now + item.path">
          ...
          <div v-if="item.isFolder" class="buttons">
            <el-button type="primary" @click="add(type_now, item.path)"
              :disabled="item.name.startsWith('#')">添加</el-button>
            <el-button type="info" @click="open(type_now, ...pathStr_2_pathArr(item.path))">打开</el-button>
          </div>
        </li>
      </ul>
