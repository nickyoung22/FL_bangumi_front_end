<template>
  <el-tooltip effect="dark" :content="fileName" :placement="this.tooltip_direction" :hide-after="0"
...
export default {
  ...
  data() {
    return {
      ...
      twitter, youtube, instagram, bilibili, internet_web, file, folder,
      picture, video
    }
  },
  computed: {
      ...
      if (this.fileName.includes('.') || this.type_appoint === 'file') {
        ...
        if (this.fileName.lastIndexOf('.url') === this.fileName.length - 1 - 3) {
          ...
          // 如果需要区分具体是哪个网站图标
            ...
            } else if (new RegExp(`(${['哔哩哔哩', 'bilibili'].join('|')})\.url$`).test(filename)) {
        ...
        // 如果是图片文件
        else if (new RegExp(`(${['.jpg', '.jpeg', '.png', '.gif'].join('|')})`).test(this.fileName.toLowerCase())) {
          return this.picture
        }
        // 如果是视频文件
        else if (new RegExp(`(${['.avi', '.mp4', '.mkv', '.wmv', '.wov'].join('|')})`).test(this.fileName.toLowerCase())) {
          return this.video
        }
        // 未知文件
        else {
